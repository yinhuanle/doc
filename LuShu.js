 ...
 * 用户可以在地图上自定义轨迹运动
 * 可以自定义路过某个点的图片，文字介绍等。
 * 主入口类是<a href="symbols/BMapLibLuShu.LuShu.html">LuShu</a>，
 * 基于Baidu Map API 1.2。.
 *
 ...
 */
var BMapLibLuShu = (window.BMapLibLuShu = BMapLibLuShu || {});
...
(function () {
  ...
  /**
   * @exports LuShu as BMapLibLuShu.LuShu
   */
  var LuShu =
    /**
         * LuShu类的构造函数
         ...
         * @constructor
             ...
             * }<br />.
             * @example <b>参考示例：</b><br />
             * var lushu = new BMapLibLuShu.LuShu(map,arrPois,{defaultContent:"从北京到天津",landmarkPois:[]});
         */
    (BMapLibLuShu.LuShu = function (map, path, opts) {
      ...
      if (opts.onMove) this.onMove = opts.onMove; // 监听移动时
      if (opts.onMoveNext) this.onMoveNext = opts.onMoveNext; // 监听移动到下一个点时
      if (opts.onFirst) this.onFirst = opts.onFirst; // 监听第一次点击时
      ...
      BMapLibLuShu.initCustomOverlay();
    });
  /**
  ...
  //等BMap加载完毕再初始化
  BMapLibLuShu.initCustomOverlay = function () {
    CustomOverlay.prototype = new BMap.Overlay();
    CustomOverlay.prototype.initialize = function (map) {
      var div = (this._div = baidu.dom.create("div", {
        style:
          "border:solid 1px #ccc;width:auto;min-width:210px;font-size: 14px;position:absolute;background:#fff;color:#000;padding:20px 15px 15px 15px;white-space: nowrap;",
      }));
    ...
    baidu.object.extend(CustomOverlay.prototype, {
      ...
      setPosition: function (poi, markerSize) {
        ...
        this._div.style.left = px.x - overlayW / 2 + 110 + "px";
        this._div.style.bottom = -(px.y - markerSize.height) + 5 + "px";
      },
      //设置overlay的内容
